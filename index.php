<?php
include('pages/components/messages.php');
$urlToRedirect='/pages/actions/santaClaus.php';
$title = MSGholidayStudio;
$content = '
<div class="row">
    <div class="small-12 medium-12 large-12 columns">

    </div>
</div>
<div class="row">
    <div class="small-12 medium-12 large-12 columns">
        <h3 class="text-center">ДОБРО ПОЖАЛОВАТЬ В СТУДИЮ ПРАЗДНИКОВ</h3>
        <h3 class="text-center">' . MSGapplicationTitleQuotes . '</h3>
    </div>
</div>
<div class="row valign-middle">
    <div class="small-12 medium-7 large-6 columns text-center">
        Ищите оригинальное признание в любви?
        ' .MSGalterApplicationTitle . ' ' . MSGapplicationTitleQuotes . ' поможет Вам сделать удивительное признание для своей второй половинки!
    </div>
    <div class="small-12 medium-5 large-3 columns text-center">
        <a class="" href="">
            <img class="home-image" src="/images/balloons/balloon_bouquet.png">
        </a>
    </div>
</div>
<div class="row valign-middle">
    <div class="small-12 medium-5 large-3 columns text-center">
        <a class="" href="">
            <img class="home-image" src="/images/clown.png">
        </a>
    </div>
    <div class="small-12 medium-7 large-6 columns text-center">
        Желаете порадовать своего ребенка?
        ' .MSGalterApplicationTitle . ' ' . MSGapplicationTitleQuotes . ' поможет Вам в этом!
    </div>
</div>
<div class="row valign-middle">
    <div class="small-12 medium-7 large-6 columns text-center">
        Хотите устроить великолепный праздник?
        ' .MSGalterApplicationTitle . ' ' . MSGapplicationTitleQuotes . ' наполнит Ваш праздник счастьем!
    </div>
    <div class="small-12 medium-5 large-3 columns text-center">
        <a class="" href="">
            <img class="home-image" src="/images/party.png">
        </a>
    </div>
</div>
<div class="row valign-middle">
    <div class="small-12 medium-5 large-3 columns text-center">
        <a class="" href="">
            <img class="home-image" src="/images/gift.png">
        </a>
    </div>
    <div class="small-12 medium-7 large-6 columns text-center">
        Ищите оригинальные и креативные подарки?
        ' .MSGalterApplicationTitle . ' ' . MSGapplicationTitleQuotes . ' нашла их для Вас!
    </div>
</div>
';
include('pages/components/template.php');
?>